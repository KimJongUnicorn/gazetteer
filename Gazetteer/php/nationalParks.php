<?php
    ...
    $url = json_decode(file_get_contents("http://api.geonames.org/searchJSON?username=kimjongunicorn&country=" . $_REQUEST['country'] . "&featureCode=PRK&maxRows=20"), true);
	
	$parkMarkers = ["type"=>"FeatureCollection","features"=>array()];
	$temp = [];
	$t = [];
	...
	foreach ($data as $park) {
		...
		$t['properties']['name'] = $park['name'];
		$t['properties']['population'] = $park['population'];
		$t['properties']['wiki'] = $wikiString . $park['name'];
	...
	$parkMarkers['features'] = $temp;

	$output['status']['code'] = "200";
	$output['status']['name'] = "ok";
	$output['status']['description'] = "mission saved";
	$output['status']['returnedIn'] = (microtime(true) - $executionStartTime) / 1000 . " ms";
	$output['data'] = $parkMarkers;
